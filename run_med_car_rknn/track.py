...
T_JUNCTION_THRESHOLD = 70
BLACK_THRESHOLD = (360, 150, 50)
...
def main(shm_name, frame_ready, conn=None):
    ...
    while True:
        ...
        # 提取黑色
        black_mask = cv2.inRange(frame, (0, 0, 0), BLACK_THRESHOLD)
        black = cv2.bitwise_and(white_frame, white_frame, mask=black_mask)
        black = cv2.morphologyEx(black, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5)))
        ...
        # 检测T型路口
        if abs(angle) > 75 and offset_y > T_JUNCTION_THRESHOLD:
            isJunction = 1
        ...
        msg = [0, angle, offset_x, inter_x, inter_y, cx, cy, isJunction]
